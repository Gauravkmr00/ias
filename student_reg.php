...
if(isset($_POST['submit'])){
    ...
    $emailData = "SELECT * FROM `student_reg` WHERE email = '$email'";
    $emailQuery1 = mysqli_query($conn,$emailData);
    $email_Count = mysqli_num_rows($emailQuery1);

    $phonelData = "SELECT * FROM `student_reg` WHERE phone = '$phone'";
    $phoneQuery1 = mysqli_query($conn,$phonelData);
    $phone_Count = mysqli_num_rows($phoneQuery1);


  if($password==$cPassword){

    if($email_Count>0 || $phone_Count){
        echo "Email or Mobile Already Exit";
    }else{

        $sql = "INSERT INTO `student_reg`(`sname`, `lname`, `roll_no`, `dob`, `email`, `phone`, `address`, `fname`, `mname`, `pphone`, `pemail`, `state`, `pin`, `course`, `password`) VALUES ('$sName','$lName',' $rollNumber','$dob','$email','$phone','$address','$fName','$mName','$pPhone','$pEmail','$state','$pin','$course',' $hashPassword')";


        $sqlQuery = mysqli_query($conn,$insertData);
        if($sqlQuery==true){
           
            echo "Data has insert Successfully";
        ...
        <form action="" method="post" enctype="multipart/form-data">
          ...
          <label for="">Gender :</label>
           ...
           <br>  -->
            ...
            <button type="submit" name="submit">Submit</button> <a href="login2.php">Login</a><br><br>
